...
def draw_bboxes(image, bboxes, line_thickness):
    ...
    for (x1, y1, x2, y2, cls_id, pos_id) in bboxes:
        ...
        t_size = cv2.getTextSize(cls_id, 0, fontScale=line_thickness / 10, thickness=font_thickness)[0]
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
        cv2.rectangle(image, c1, c2, color, -1, cv2.LINE_AA)  # filled
        cv2.putText(image, '{} ID-{}'.format(cls_id, pos_id), (c1[0], c1[1] - 2), 0, line_thickness / 10,
